...
/*
 * i2c_smbus inline functions from i2c-tools i2c-dev.h
 * These are not provided by the kernel's i2c-dev.h, so we define them here.
 */
...
{
    union i2c_smbus_data data;
    if (i2c_smbus_access(file, I2C_SMBUS_READ, 0,
                         I2C_SMBUS_BYTE, &data))
        return -1;
    else
        return 0x0FF & data.byte;
